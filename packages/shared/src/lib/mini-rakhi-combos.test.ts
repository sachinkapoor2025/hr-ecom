...
describe("mini rakhi combos", () => {
  ...
  it("builds 2–5 piece set products at mix price + $1.50 list extra", () => {
    ...
    for (const product of products) {
      ...
      if (size === 2) assert.equal(product.price, 18);
      if (size === 3) assert.equal(product.price, 18);
      if (size === 4) assert.equal(product.price, 18);
      if (size === 5) assert.equal(product.price, 18);
    }
  });
});
