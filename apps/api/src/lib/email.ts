...
Valid for: 1 hour (until ${expiryLabel} ET)
Email required at checkout: ${input.customerEmail}

Use this code at checkout (must match the email above):
${checkoutUrl}
...
${siteUrl()}`;
  ...
  const notifyText = `Admin abandoned-cart coupon generated

Customer email: ${input.customerEmail}
Phone: ${input.phone}
Coupon: ${input.code}
Discount: ${input.discountPercent}%
Expires: ${expiryLabel} ET (1 hour)
Generated by: ${input.createdByAdminEmail}
${waLine}
Customer was emailed this coupon automatically.

— ${SITE_NAME} Admin`;

  const customer = await sendEmail({
    to: input.customerEmail,
    subject: `Your ${input.discountPercent}% UsaRakhi coupon (${input.code}) — valid 1 hour`,
    text: customerText,
    replyTo: notifyAddress(),
  });
  ...
  const notify = await sendEmail({
    to: notifyTo,
    subject: `[Coupon] ${input.discountPercent}% ${input.code} → ${input.customerEmail}`,
    text: notifyText,
  });
