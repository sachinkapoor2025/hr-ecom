...
**Delivered / Complete review request (once per order):** The first time an order reaches `delivered` or `complete` (admin, vendor tracking, or USPS sync), `dispatchReviewRequest` sends a transactional **review request** by SMTP (`order@usarakhi.com`, `sendReviewRequestEmail` in `apps/api/src/lib/email.ts`) and WhatsApp. Includes delivery confirmation, a **Leave a Review** button (`/reviews` by default), and a separate **Review us on Google** button. Idempotent via `reviewEmailSentAt` and `reviewWhatsAppSentAt` (or `reviewWhatsAppSkippedAt` when there is no valid phone). Saving Delivered again or later changing Delivered → Complete does **not** resend. Status update always succeeds if email/WhatsApp fails; the 15-minute cron retries unsent channels. Settings: `CONFIG#REVIEW_REQUEST` (Admin → Marketing Email → Review request). Generic delivered/complete customer status email is skipped on that first send so the shopper is not emailed twice. From/Reply-To are `order@usarakhi.com` (never the customer). Inbox placement depends on DNS for `usarakhi.com`: SPF must authorize `smtp.usarakhi.com`, DKIM should sign mail from that host, and DMARC should be `p=none` or stricter at `_dmarc.usarakhi.com`. Confirm with `nslookup -type=txt usarakhi.com` / `_dmarc.usarakhi.com` at the DNS host — this repo cannot change DNS.
...
| POST | `/admin/orders/{orderId}/review-request` | Admin: retry unsent review-request channels (never duplicates a successful send) |
